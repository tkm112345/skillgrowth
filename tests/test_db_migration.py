...
def test_ensure_column_adds_missing_column_to_existing_table():
    ...
    with Session(engine) as session:
        session.exec(text("INSERT INTO exportsnapshot (id, content, edited_at) VALUES ('x', 'y', NULL)"))
        session.commit()
...
def test_ensure_column_is_a_no_op_when_column_already_exists():
    engine = create_engine("sqlite://", connect_args={"check_same_thread": False})
    with engine.connect() as conn:
        conn.exec_driver_sql("CREATE TABLE exportsnapshot (id TEXT PRIMARY KEY, content TEXT, edited_at TIMESTAMP)")
        conn.commit()
